...
const client = new Client({
  ...
  pairWithPhoneNumber: BOT_PHONE_NUMBER ? {
...
let pairingFallbackRequested = false;

client.on('qr', async () => {
  console.log('QR RECEIVED. Requesting phone-number pairing code explicitly...');
  if (!BOT_PHONE_NUMBER || pairingFallbackRequested) return;
  pairingFallbackRequested = true;
  try {
    const code = await client.requestPairingCode(BOT_PHONE_NUMBER, true, 180000);
    console.log('==============================================');
    console.log(`PAIRING CODE (explicit): ${code}`);
    console.log(`PAIRING PHONE: ${BOT_PHONE_NUMBER}`);
    console.log('WhatsApp > Linked devices > Link a device > Link with phone number');
    console.log('==============================================');
  } catch (err) {
    pairingFallbackRequested = false;
    console.error('PAIRING_CODE_REQUEST_ERROR:', err?.stack || err);
  }
});
...
client.initialize().catch(err => {
  console.error('WHATSAPP_INITIALIZE_ERROR:', err?.stack || err);
  process.exitCode = 1;
});
